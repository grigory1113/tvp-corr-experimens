...
class OilRubleExperiment:
    """Experiment for estimating the correlation between oil and ruble"""
    ...
    def load_and_prepare_data(self):
        ...
        self.data = DataProcessor.load_and_prepare_data(
        ...
        # Add variance to data for visualization
    ...
    def run_tvp_algorithm(self):
        ...
        # Compute correlation taking into account x variance
        ...
        a_mean = np.mean(a_post, axis=0)  # (n, 2)
        
        sigma_t_post = tvp_results['sigma_t_est_history'][burn_in:, :]
        sigma_t_mean = np.sqrt(np.mean(sigma_t_post, axis=0))  # (n,)
        
        # Compute residuals: z - a0 - a1*x_raw
    ...
    def run_dcc_garch(self):
        ...
        # Prepare data for DCC-GARCH
        rets = pd.DataFrame({
            'oil': self.data['z'].flatten(),
    ...
    def plot_results(self, tvp_results, dcc_results, rolling_corr):
        ...
        # 1. Original data
        ax1 = axes[0]
        ax1.plot(self.data['dates'], self.data['oil_growth'], 
                label='Oil price change, %', 
                color=colors['oil'], alpha=0.7)
        ...
        ax2.set_title('Dynamic correlation between oil price and ruble exchange rate')
        ax2.set_ylim(-1.1, 1.1)

        # Horizontal line at zero
        ax2.axhline(y=0, color='black', linestyle='-', linewidth=0.5, alpha=0.5)

        # Shading crisis periods (example)
        crisis_periods = [
            ('1998-08-01', '1999-01-01', 'Crisis 1998'),
    ...
    def test_normality(self, standardized_residuals):
        ...
        # Shapiro-Wilk test (for samples up to 5000)
        ...
        # Jarque-Bera test (suitable for large samples)
        ...
        # Proportion of observations outside ±2σ and ±3σ
        ...
        # Q-Q plot statistic (correlation coefficient)
    ...
    def save_results(self, tvp_results, dcc_results, rolling_corr, output_dir='results/oil_ruble'):
        """Save results"""
        import os
        import json
    ...
    def run_full_experiment(self):
        """Run full experiment"""
        print("=" * 70)
        print("EXPERIMENT: OIL AND RUBLE CORRELATION")
        print("=" * 70)
        ...
        fig_raw = DataProcessor.plot_raw_data(self.data)
        fig_raw.savefig('oil_ruble_raw_data.png', dpi=300, bbox_inches='tight')
        plt.show()
        
        # 3. Run TVP algorithm
        tvp_results = self.run_tvp_algorithm()
        
        # 4. Normality tests on residuals
        normality_results = self.test_normality(tvp_results['results']['standardized_residuals'])
        
        # 5. Visualize standardized residuals
        print("\nVisualizing standardized residuals...")
        fig_residuals = self.plot_standardized_residuals(tvp_results)
        fig_residuals.savefig('oil_ruble_standardized_residuals.png', dpi=300, bbox_inches='tight')
        plt.show()
        
        # 6. Run DCC-GARCH
        dcc_results = self.run_dcc_garch()
        
        # 7. Calculate rolling correlation
        rolling_corr = self.calculate_rolling_correlation()
        
        # 8. Visualize results
        fig_results = self.plot_results(tvp_results, dcc_results, rolling_corr)
        fig_results.savefig('oil_ruble_correlation_comparison.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def main():
    """Main function"""
    config = OilRubleConfig()
    experiment = OilRubleExperiment(config)
